...
void simuStateCallback(const test_controller::UAVState::ConstPtr &msg, common::MyState &state)
{
  state.p = Eigen::Vector3d(msg->position.x, msg->position.y, msg->position.z);
  state.vi = Eigen::Vector3d(msg->velocity.x, msg->velocity.y, msg->velocity.z);
  state.q =  Eigen::Vector4d(msg->attitude.w, msg->attitude.x, msg->attitude.y, msg->attitude.z);
  state.R = Eigen::Quaterniond(state.q(0), state.q(1), state.q(2), state.q(3)).toRotationMatrix(); //q: w x y z
  state.euler = state.R.eulerAngles(2, 1, 0);
  state.updated = true;
...
int main(int argc, char **argv)
{
  ...
  // 加载参数文件
  if (!params.loadFromFile("/home/fran/catkin_ws/src/test_controller/json/myparams.json")) {
  ...
  if(!simu){
    ...
    ros::Subscriber state_sub_local = nh.subscribe<std_msgs::Float64MultiArray>("state", 10,
    ...
    ros::Subscriber feedback_sub = nh.subscribe<test_controller::UAVState>(uav_id + "/feedback", 10, std::bind(simuStateCallback, std::placeholders::_1, std::ref(state)));
    ...
    while (ros::ok())
    {
      ...
      command_msg.thrust = control_signal(0);
      command_msg.omega.x = control_signal(1);
      command_msg.omega.y = control_signal(2);
      command_msg.omega.z = control_signal(3);
